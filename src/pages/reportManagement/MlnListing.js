...
const MlnListing = () => {
  ...
  const { state: { apiKey = "", title = "", date = "", endDate = "" } = {} } =
    location;

  const getData = async (currentPage) => {
    const res = await axios.get(
      `${baseURL}ticket_management/${apiKey}/?page=${currentPage}&date=${date}&to=${endDate}`,
      config
    );
  ...
  const COLUMNS = [
    ...
    {
      Header: "Token Average",
      accessor: (d) => {
        const sgd = +d?.token_avg;
        return <div style={{ textAlign: "center" }}> {sgd?.toFixed(3)} </div>;
      },
    },
    // {
