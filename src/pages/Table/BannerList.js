...
const BannerList = ({
  ...
  const getBanners = async (currentPage) => {
    const res = await axios.get(
      `${baseURL}privacy/banners/?page=${currentPage}`,
      config
    );
    //("resp", res);
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: (cell) => {
        return (
          <a href={cell?.value} target="blank">
            {cell?.value}
          </a>
        );
      },
              ...
              >
                ...
                <MenuItem>
                  <NavLink
                    ...
                    // to={`/mooner/edit_banner/${record.value}`}
                    className={classes.links}
                  >
                    <div className={classes.FlexWrapper}>
